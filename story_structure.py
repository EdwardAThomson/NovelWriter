...
import re
from helper_fns import open_file, write_file, read_json, save_prompt_to_file
import os
import traceback
import json
from parameters import STRUCTURE_SECTIONS_MAP # Import the centralized map
import logging
...
class StoryStructure:
    ...
    def _dispatch_detailed_plot_creation(self):
        ...
        if not (self.app and hasattr(self.app, 'param_ui')):
            ...
            messagebox.showerror("Error", "Cannot determine story parameters.")
            return
        ...
        if story_length == "Short Story":
            ...
            messagebox.showerror("Error", f"Unsupported story length '{story_length}' for this operation.")
    ...
    def generate_arcs(self):
        ...
        try:
            ...
            if not all_characters_data:
                ...
                messagebox.showerror("Error", f"No characters loaded from {characters_json_path}. Required for identifying main character backstories.")
                return
            ...
            messagebox.showerror("Error", f"Characters file not found: {characters_json_path}")
            return
        except (json.JSONDecodeError, ValueError) as e:
            self.app.logger.error(f"Error loading or parsing {characters_json_path}: {e}", exc_info=True)
            messagebox.showerror("Error", f"Error parsing characters file: {characters_json_path}")
            return
        ...
        try:
            ...
            try:
                ...
                messagebox.showwarning("Missing File", f"Lore file not found: {lore_file_path}. Arcs might lack context.")
            ...
            if not main_chars_with_backstories:
                ...
                messagebox.showerror("Error", "No main character background files could be loaded. Check logs for details. Cannot generate arcs.")
                return
            ...
            if not response:
                 ...
                 messagebox.showerror("Error", "Failed to generate character arcs from LLM.")
                 return
            ...
            messagebox.showinfo("Success", f"Character arcs generated and saved to {output_file_path}")

        except Exception as e:
            self.app.logger.error(f"Failed to generate character arcs: {e}", exc_info=True)
            # traceback.print_exc() # Handled by logger
            messagebox.showerror("Error", f"An unexpected error occurred during character arc generation: {str(e)}")
    ...
    def generate_faction_arcs(self):
        ...
        try:
            ...
            try:
                ...
                messagebox.showerror("Error", f"Character arcs file not found: {character_arcs_file_path}. Cannot proceed.")
                return
            
            try:
                lore_content = open_file(lore_file_path)
            except FileNotFoundError:
                messagebox.showwarning("Missing File", f"Lore file not found: {lore_file_path}. Faction arcs might lack context.")
            ...
            try:
                ...
                if not major_factions:
                    ...
                    messagebox.showerror("Error", f"No faction data found or extracted from {factions_json_file_path}.")
                    return
                ...
                messagebox.showerror("Error", f"Factions JSON file not found: {factions_json_file_path}. Cannot proceed.")
                return
            ...
            if not faction_arcs_response:
                ...
                messagebox.showerror("Error", "Failed to generate faction arcs from LLM.")
                return
            ...
            if not reconciled_response:
                ...
                messagebox.showerror("Error", "Failed to reconcile arcs using LLM.")
                return
            ...
            messagebox.showinfo("Success", f"Faction arcs generated and reconciled arcs saved to {reconciled_output_file_path}")
    ...
    #         messagebox.showinfo("Success", f"Story structure ({selected_structure}) saved to {story_structure_output_path}")

    #     except FileNotFoundError as fnf_e:
    #         messagebox.showerror("Error", f"File not found during structure generation: {fnf_e}")
    ...
    #         messagebox.showinfo("Success", f"Locations added to structure. Saved to {os.path.basename(story_structure_locations_output_path)}")

    #     except FileNotFoundError as fnf_e:
    #         messagebox.showerror("Error", f"File not found: {fnf_e}")
    #         print(f"File not found: {fnf_e}")
    #     except Exception as e:
    #         print(f"Failed to generate story structure with locations: {e}")
    #         traceback.print_exc()
    #         messagebox.showerror("Error", f"Failed to generate story structure with locations: {str(e)}")
    ...
    def improve_structure(self):
        ...
        if not sections_to_iterate:
            messagebox.showerror("Error", f"No defined sections for structure '{selected_structure_name}'. Cannot improve structure.")
        ...
        try:
            ...
            messagebox.showinfo("Success", f"Detailed sections for '{selected_structure_name}' generated.")

        except FileNotFoundError as fnf_e:
            messagebox.showerror("Error", f"File not found during structure improvement: {fnf_e}")
            ...
            messagebox.showerror("Error", f"Failed to improve story structure: {str(e)}")
    ...
    def _outline_short_story_plot(self):
        ...
        if not (self.app and hasattr(self.app, 'param_ui')):
            ...
            messagebox.showerror("Error", "Cannot load story parameters.")
            return
        ...
        if not selected_structure_name:
            ...
            messagebox.showerror("Error", "No story structure selected. Please select one in Novel Parameters.")
            return
        ...
        if not structure_stages:
            ...
            messagebox.showerror("Error", f"Cannot find definition for structure '{selected_structure_name}'.")
            return
        ...
        if not response:
            ...
            messagebox.showerror("Error", "Failed to generate short story plot from LLM.")
            return
        ...
        try:
            ...
            messagebox.showinfo("Success", f"Short story plot generated and saved to {output_filename_full_path}")
        except Exception as e:
            self.app.logger.error(f"Error saving short story plot to {output_filename_full_path}: {e}", exc_info=True)
            messagebox.showerror("Error", f"Failed to save short story plot: {e}")
