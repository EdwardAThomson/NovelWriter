...
import re
from helper_fns import open_file, write_file, save_prompt_to_file, read_json
import os
from parameters import STRUCTURE_SECTIONS_MAP # Import for section mapping
...
class ChapterWriting:
    ...
    def _dispatch_prose_generation(self):
        ...
        if not (self.app and hasattr(self.app, 'param_ui')):
            ...
            messagebox.showerror("Error", "Cannot determine story parameters for prose generation.")
            return
        ...
        if story_length == "Short Story":
            ...
            messagebox.showerror("Error", f"ChapterWriting: Unsupported story length '{story_length}'.")
    ...
    def _write_short_story_prose(self):
        ...
        try:
            ...
            if not selected_structure_name:
                ...
                messagebox.showerror("Error", "No story structure selected in parameters.")
                return
            ...
            try:
                ...
                if not scene_plan_content.strip():
                    ...
                    messagebox.showerror("Error", f"The scene plan file '{scene_plan_filename}' is empty. Cannot generate prose.")
                    return
            except FileNotFoundError:
                self.app.logger.error(f"Scene plan file not found: {scene_plan_filepath}")
                messagebox.showerror("Error", f"Scene plan file '{scene_plan_filename}' not found. Please generate scenes first via the 'Scene Planning' tab.")
                return
            except Exception as e:
                self.app.logger.error(f"Error reading scene plan file '{scene_plan_filepath}': {e}", exc_info=True)
                messagebox.showerror("Error", f"Could not read the scene plan file: {e}")
                return
            ...
            if not parsed_scenes:
                ...
                messagebox.showerror("Error", "Could not parse scenes from the plan. Ensure scenes start with '### Scene X: ...' or '**Scene X: ...**'.")
                return
            ...
            if not all_generated_prose:
                ...
                messagebox.showerror("Error", "No prose could be generated for the short story.")
                return
            ...
            try:
                ...
                messagebox.showinfo("Success", f"Short story prose generated and saved to {output_story_filename}")
            except Exception as e_write:
                self.app.logger.error(f"Error writing full short story to file '{output_story_filepath}': {e_write}", exc_info=True)
                messagebox.showerror("Error", f"Failed to save the complete short story: {e_write}")
    ...
    def write_chapter(self):
        ...
        try:
            ...
            if target_chapter_number_global <= 0:
                messagebox.showerror("Error", "Chapter number must be a positive integer.")
                return
            ...
            try:
                if not os.path.exists(parameters_file_path):
                    ...
                    messagebox.showerror("Error", f"Parameters file 'parameters.txt' not found in {output_dir}.")
                    return
            ...
            if not sections_to_process:
                ...
                messagebox.showerror("Error", f"Cannot find structure definition for '{selected_structure_name}'.")
                return
            ...
            if not current_section_name_for_chapter:
                ...
                messagebox.showerror("Error", f"Could not locate chapter {target_chapter_number_global} within the known structure sections. Ensure all chapter outlines are generated.")
                return
            ...
            try:
                ...
                if not scenes_content_for_chapter.strip():
                    ...
                    messagebox.showerror("Error", f"Scene plan file for Chapter {target_chapter_number_global} is empty.")
                    return
            except FileNotFoundError:
                self.app.logger.error(f"Scene plan file not found: {scene_plan_filepath}")
                messagebox.showerror("Error", f"Scene plan file for Chapter {target_chapter_number_global} not found in '{scene_plans_subdir_name}'. Ensure scenes are planned.")
                return
                ...
                messagebox.showerror("Error", f"Could not read scene plan for Chapter {target_chapter_number_global}: {e_read_scenes}")
                return
            ...
            if not scene_matches_in_chapter:
                ...
                messagebox.showerror("Error", f"No individual scenes found in the plan for Chapter {target_chapter_number_global}.")
                return
            ...
            messagebox.showinfo("Success", f"Chapter {target_chapter_number_global} generated and saved to {chapter_filename_output}")
            ...
            messagebox.showerror("Error", f"Failed to write chapter: {str(e_main)}")
    ...
    def rewrite_chapter(self):
        ...
        try:
            ...
            try:
                ...
                messagebox.showerror("Error", f"Chapter file '{chapter_filename_input}' not found to rewrite.")
                return
                ...
                messagebox.showerror("Error", f"Could not read chapter for rewrite: {e_read_rewrite}")
                return
            ...
            messagebox.showinfo("Success", f"Chapter {chapter_number} RE-saved successfully to {output_rewrite_filename}")

        except ValueError:
            self.app.logger.error("Invalid chapter number entered for rewrite.", exc_info=True)
            messagebox.showerror("Error", "Please enter a valid chapter number for rewrite.")
            ...
            messagebox.showerror("Error", f"Failed to RE-write chapter: {str(e_rewrite_main)}")
