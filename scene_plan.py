...
import re
from helper_fns import open_file, write_file, save_prompt_to_file
import os
from parameters import STRUCTURE_SECTIONS_MAP
...
class ScenePlanning:
    ...
    def _dispatch_scene_planning(self):
        ...
        if not (self.app and hasattr(self.app, 'param_ui')):
            ...
            messagebox.showerror("Error", "Cannot determine story parameters for scene planning.")
            return
        ...
        if story_length == "Short Story":
            ...
            messagebox.showerror("Error", f"ScenePlanning: Unsupported story length '{story_length}'.")
    ...
    def generate_chapter_outline(self):
        ...
        if not sections_to_process:
            messagebox.showerror("Error", f"Section definitions for '{selected_structure_name}' not found.")
        ...
        try:
            ...
            for current_section_name in sections_to_process:
                ...
                try:
                    ...
                    messagebox.showwarning("Missing File", f"Detailed plan for section '{current_section_name}' (file: {input_filename_base}) not found. Skipping this section.")
            ...
            messagebox.showinfo("Success", f"Chapter outlines for '{selected_structure_name}' generated successfully.")

        except FileNotFoundError as fnf_e: # Should be caught per-file above, but as a fallback
            messagebox.showerror("Error", f"A required file was not found: {fnf_e}")
    ...
    def scene_plan(self):
        ...
        if not sections_to_process:
            messagebox.showerror("Error", f"Scene Plan: Section definitions for '{selected_structure_name}' not found.")
        ...
        try:
            ...
            try:
                ...
                messagebox.showwarning("Missing File", f"Lore file {lore_content_path} not found. Scene plans may lack context.")
            ...
            for current_section_name in sections_to_process:
                ...
                try:
                    ...
                    messagebox.showwarning("Missing File", f"Chapter outline file for section '{current_section_name}' (file: {chapter_outline_input_base}) not found. Skipping scene planning for this section.")
            ...
            messagebox.showinfo("Success", f"Scene plans for '{selected_structure_name}' generated successfully.")

        except FileNotFoundError as fnf_e:
            messagebox.showerror("Error", f"Scene Plan: A required file was not found: {fnf_e}")
            print(f"Error (Scene Plan): File not found - {fnf_e}")
        except Exception as e:
            print(f"Failed to generate scene plans: {e}")
            import traceback
            traceback.print_exc()
            messagebox.showerror("Error", f"Failed to generate scene plans: {str(e)}")
    ...
    def _plan_short_story_scenes(self):
        ...
        if not selected_structure_name:
            ...
            messagebox.showerror("Error", "No story structure. Please select one in Novel Parameters.")
            return
        ...
        try:
            ...
            if not short_story_plot_content.strip():
                ...
                messagebox.showerror("Error", f"The detailed plot file '{detailed_plot_filename}' is empty. Cannot plan scenes.")
                return
        except FileNotFoundError:
            self.app.logger.error(f"Detailed short story plot file not found: {detailed_plot_filepath}")
            messagebox.showerror("Error", f"Detailed plot file '{detailed_plot_filename}' not found. Please generate it first using the Story Structure tab.")
            return
        except Exception as e:
            self.app.logger.error(f"Error loading detailed short story plot file '{detailed_plot_filepath}': {e}", exc_info=True)
            messagebox.showerror("Error", f"Could not read '{detailed_plot_filename}'.")
            return
        ...
        if not response:
            ...
            messagebox.showerror("Error", "Failed to generate short story scenes from LLM.")
            return
        ...
        try:
            ...
            messagebox.showinfo("Success", f"Short story scenes generated and saved to {output_filename_full_path}")
        except Exception as e:
            self.app.logger.error(f"Error saving short story scenes to {output_filename_full_path}: {e}", exc_info=True)
            messagebox.showerror("Error", f"Failed to save short story scenes: {e}")
